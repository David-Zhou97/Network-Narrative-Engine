...
import { storyRegistry, getAllStories, getUserStories, type StoryInfo } from '../data/storyRegistry';
...
export function StoryMarketplace(): React.ReactElement {
  ...
  const [allStoriesList, setAllStoriesList] = useState<StoryInfo[]>(getAllStories());

  // Refresh stories list when component mounts or when returning from story creator
  useEffect(() => {
    setAllStoriesList(getAllStories());
  }, []);

  const allTags = Array.from(
    new Set(allStoriesList.flatMap(story => story.tags))
  ).sort();

  const filteredStories = filter === 'all'
    ? allStoriesList
    : allStoriesList.filter(story => story.tags.includes(filter));

  const featuredStories = allStoriesList.filter(s => s.featured);
  ...
  return (
    <div className={styles.container}>
      ...
      {filter === 'all' && userStories.length > 0 && (
        <motion.section
          className={styles.userStoriesSection}
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
        ...
        >
          ...
          <div className={styles.storiesGrid}>
            {userStories.map((story, index) => (
              <motion.button
                key={story.id}
                className={`${styles.storyCard} ${styles.userStoryCard}`}
                onClick={() => handleStoryClick(story)}
                initial={{ opacity: 0, y: 20 }}
                animate={{ opacity: 1, y: 0 }}
              ...
              >
                <div className={styles.storyThumbnail}>
                  <img src={story.thumbnail} alt={story.title} />
                  <div className={styles.userBadge}>Your Story</div>
                </div>
